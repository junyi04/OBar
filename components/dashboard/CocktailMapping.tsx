...
export const CocktailMapping = () => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cold-s-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {cocktails.map((cocktail) => (
        <Link href={`/detail/${cocktail.id}`} key={cocktail.id}>
          <div className="bg-slate-800 rounded-lg shadow-lg p-4 cursor-pointer hover:bg-slate-700 transition duration-300">
            ...
            <p className={`mt-2 text-sm font-semibold ${cocktail.alcoholic === 'Alcoholic' ? 'text-red-400' : 'text-green-400'}`}>
              {cocktail.alcoholic}
            </p>
          </div>
        </Link>
